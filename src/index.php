...
例えば、ログインフォームにおいて、ユーザーがユーザー名とパスワードを提供し、その情報をデータベースと照合して認証するとします。
しかし、入力を適切に検証せずにSQLクエリを構築する場合、攻撃者は不正なSQLコードを入力し、データベースに対して実行される可能性があります。
例えば、次のようなクエリが考えられます：
...
SQLインジェクションは深刻なセキュリティリスクであり、悪用されると機密データの漏洩、データベースの破壊、システムへの不正アクセスなどの深刻な影響を引き起こす可能性があります。
そのため、適切な対策を講じることが重要です。*/


/*プリペアドステートメント（Prepared Statement）は、SQLインジェクションを防ぐための効果的な手法の1つです。通常、SQLクエリは実行される前にデータベースエンジンによって解析され、最適な実行プランが決定されます。
しかし、プリペアドステートメントでは、SQLクエリの構文がデータベースに送信される前に、あらかじめ解析され、最適化された実行プランが準備されます。
...
このプロセスにより、データベースエンジンはクエリを事前に準備し、バインドされる値が入力される前にクエリを解析するため、SQLインジェクション攻撃から保護されます。
また、同じクエリを複数回実行する場合、プリペアドステートメントを使用することで、データベースエンジンはクエリの解析、コンパイル、最適化のオーバーヘッドを一度だけ行います。*/

/*小学生にも分かるver
プリペアドステートメントは、データベースに特別な命令を送る方法の一つです。これは、データベースに対して安全に質問をするためのトリックです。
プリペアドステートメントを使うと、まず質問の形を決めますが、具体的な答えの部分は空っぽにしておきます。そして、その質問をデータベースに送ります。
データベースはその質問を受け取ると、質問の形だけを覚えておいて、具体的な答えは後で入れてもらうように待ちます。
...
具体的な例を考えてみましょう。データベースにユーザーの情報を追加するとき、名前や年齢などの情報が必要ですが、その情報はユーザーごとに異なります。
そこで、SQLクエリ内の名前や年齢の部分には、特別な場所（プレースホルダー）を用意しておきます。
...
データベースは、バインドされたデータをクエリ内の対応する場所に挿入し、クエリを実行します。これによって、プレースホルダーを使って安全にSQLクエリを構築し、実際のデータを安全にデータベースに送ることができます。
そして、SQLインジェクションなどの攻撃からデータベースを守ることができます。*/
